<template>
    ...
    <div class="w-[90%] m-auto">
        <div >
            <div class="relative">
                <div class="w-full ">
                    <img v-if="user.coverimage == null" class="w-full h-52 border border-1 border-gray-300" src="https://img.freepik.com/free-photo/hand-painted-watercolor-background-with-sky-clouds-shape_24972-1095.jpg?w=900&t=st=1662803337~exp=1662803937~hmac=cf60bab2657205cd3fee65a76ce66d234eb8ec4d516ec6f7c62257a2cfefd8d5" alt="">
                    <img v-if=" !isUpdateCover && user.coverimage != null" class="w-full h-52 border border-1 border-gray-300" :src="'http://127.0.0.1:8000/images/Cover/'+ user.coverimage" alt="">
                ...
                <div v-if="isUpdateCover" class="flex justify-end mt-[-60px] absolute right-0" >
                    <button @click="isUpdateCover = !isUpdateCover" class="btn w-[8rem] bg-gray-300 rounded p-2 w-full mr-3">
                        Cancel
                    </button>
                    <button @click="saveCover()" class="btn w-[8rem] bg-[#22bbea] rounded p-2 text-white w-full mr-5">
                        Save Change
                    </button>
                </div>
...
export default {
    ...
    methods:{
        ...
        axios.post("/alumniprofile/" + 12, formData).then(() => {
            this.getUser();
            this.isUpdate=false;
        });
        },
