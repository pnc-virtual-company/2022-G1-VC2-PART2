<template>
<form class="border-2 border-sky-400 w-[76%] m-auto mt-5 rounded-sm" >
    <table class=" table-auto w-[98%] flex flex-wrap m-auto mt-5" >
        <thead class="w-[95%] flex flex-wrap m-auto shadow-sm p-4" >
            <tr class="w-[25%] m-auto">
                <th><slot></slot></th>
            </tr>
            <tr class="w-[35%] m-auto">
                <th>LOCATION</th>
            </tr>
            <tr class="w-[9%] m-auto">
                <th>ACTION</th>
            </tr>
        </thead>
        <tbody v-for:="company in companies" class="w-[95%] flex flex-wrap m-auto shadow-sm p-4" slot="tbody" >
                <tr class="w-[35%] m-auto" t>
                    <td class="w-[30%] m-1">
                        <div class="w-16 h-16 border-2 border-blue-500  rounded-full">
                            <img src="../../assets/logo.png" class="w-[70%] m-auto mt-3">
                        </div>
                    </td>
                    <td>
                        <p>{{company.name}}</p>
                    </td>
                </tr>
                <tr class="w-[45%] m-auto">
                    <td class="w-[40%] m-auto">
                        <p>{{company.address}}</p>
                    </td>
                </tr>
                <tr @click ="removeCompany(company.id)" class="w-[10%] m-auto">
                    <td class="w-[100%]">
                        <div class="w-[40%] bg-red-500 rounded-md cursor-pointer p-1 shadow-sm border-b">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" class="w-[50%] m-auto" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                            </svg>
                        </div>
                    </td>
                </tr>
        </tbody>
    </table>
 </form>
</template>

<script>
import axios from '../../axios-http'
export default {
    data(){
        return {
           companies:[],
        }
    },
    methods:{
        removeCompany(company){
         axios.delete('companies/'+company);
         this.getCompany();
        },
        getCompany(){
            axios.get('companies').then((response)=>{
                this.companies = response.data;
            })
        }
    },
    mounted(){
        this.getCompany();
    }
}
</script>