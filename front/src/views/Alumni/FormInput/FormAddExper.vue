    ...
    <div tabindex="-1" class=" bg-[#000000b9] fixed  flex items-center z-50 md:inset-0 h-modal md:h-full">
        <div class="modal bg-white h-auto shadow-md rounded p-5 mt-2 mb-10 m-auto w-[30rem] z-10">
            ...
            <div v-if="companies!=null" class="w-[100%] my-2 flex items-center p-2">
                ...
                <select v-model="companyId" class="block p-2 w-full outline-none text-gray-900 bg-gray-100 rounded-sm border border-gray-300 sm:text-xs focus:ring-skyblue" :class="msError['require_error']?'border-red-400':''">
            ...
            <div class="w-[100%] flex my-2 items-center p-2">
                ...
                <input v-model="position" type="text" placeholder="require*" class="block font-xl p-2 w-full outline-none text-gray-900 bg-gray-100 rounded-sm border border-gray-300 sm:text-xs focus:ring-skyblue" :class="msError['require_error']?'border-red-400':''">
            </div>
            ...
            <div class="start-date">
                ...
                <div class="date flex justify-between my-2 p-0">
                    <div class="w-[100%] mx-2 text-start">
                        <select v-model="start_month" class="block p-2 w-full outline-none text-gray-900 bg-gray-50 rounded-sm border border-gray-300 sm:text-xs focus:ring-skyblue" :class="msError['require_error']?'border-red-400':''">
                            <option value="" disabled>Month</option>
                            <option v-for:= "month of months" :value="month">{{month}}</option>
                        </select>
                    </div>
                    <div class="w-[100%] mx-2 text-start">
                        <select v-model="start_year" class="block p-2 w-full outline-none text-gray-900 bg-gray-50 rounded-sm border border-gray-300 sm:text-xs focus:ring-skyblue" :class="msError['require_error']?'border-red-400':''">
             ...
             <div :class="ischeckboxed?'hidden':'end_date'">
                ...
                <div class="date flex justify-between my-2 p-0">
                    <div class="w-[100%] mx-2 text-start">
                        <select v-model="end_month" class="block p-2 w-full outline-none text-gray-900 bg-gray-50 rounded-sm border border-gray-300 sm:text-xs focus:ring-skyblue" :class="msError['require_error']?'border-red-400':''">
                            <option value="" disabled>Month</option>
                            <option v-for:= "month of months" :value="month">{{month}}</option>
                        </select>
                    </div>
                    <div class="w-[100%] mx-2 text-start">
                        <select v-model="end_year" class="block p-2 w-full outline-none text-gray-900 bg-gray-50 rounded-sm border border-gray-300 sm:text-xs focus:ring-skyblue" :class="msError['require_error']?'border-red-400':''">
                            <option value="" disabled>Year</option>
                            <option v-for:= "year of years" :value="year">{{year}}</option>
                        </select>
                    </div>
