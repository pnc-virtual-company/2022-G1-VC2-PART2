        ...
        <div v-for:="(experience, i) in AlumniExper"  class="flex justify-between mt-2 p-2 border-b-[1px] border-[#a9aaaaa3]">
            ...
            <div>
                ...
                <div v-if="isClick && id == i" class="absolute bg-slate-300 rounded mt-[-30px] ml-[30px] text-[14px]"  v-click-outside="onClickOutSide">
                    <p class="flex items-center hover:cursor-pointer hover:bg-skyblue py-1 px-3 hover:text-white hover:rounded-t" @click="$emit('editor', experience), isClick=false">
                        <span class="mr-1">
                    ...
                    <p class="flex items-center hover:cursor-pointer hover:bg-red-500 py-1 px-3 hover:text-white hover:rounded-b" @click="$emit('removeExper',experience.id),isClick=false">
                        <span class="mr-1">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
